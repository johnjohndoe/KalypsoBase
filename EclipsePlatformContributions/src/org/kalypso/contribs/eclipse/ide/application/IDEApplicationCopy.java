...
/**
 * This is actually a copy of {@link org.eclipse.ui.internal.ide.application.IDEApplication}, but changed so that it can
 * be used from several Kalypso based products.<br/>
 * Also observe the following:
 * <ul>
 * <li>Do make sure NOT to start any plugins that access the workspace area, else the instance-dialog will fail.</li>
 * </ul>
 * <br/>
 * Original class comment:<br/>
 * This class needs to be recreated on every new eclipse version.
 * 
 * @author Gernot Belger
 */
@SuppressWarnings("restriction")
public abstract class IDEApplicationCopy implements IApplication, IExecutableExtension
{
...
// }
  ...
  private boolean checkInstanceLocation( final Shell shell )
  {
    ...
    final ChooseWorkspaceData launchData = new ChooseWorkspaceData( instanceLoc.getDefault() );
  ...
  private boolean checkValidWorkspace( final Shell shell, final URL url )
  {
    ...
    final String version = readWorkspaceVersion( url );
    ...
    // to use it anyhow.
    final String title = IDEWorkbenchMessages.IDEApplication_versionTitle;
    final String message = NLS.bind( IDEWorkbenchMessages.IDEApplication_versionMessage, url.getFile() );
  ...
  private static void writeWorkspaceVersion( )
  {
    ...
    try
    {
      final String versionLine = WORKSPACE_VERSION_KEY + '=' + WORKSPACE_VERSION_VALUE;
  ...
  private static File getVersionFile( final URL workspaceUrl, final boolean create )
  {
    ...
    try
    {
      ...
      final File metaDir = new File( workspaceUrl.getPath(), IDEApplication.METADATA_FOLDER );
