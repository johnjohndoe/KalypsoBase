 ...
 *******************************************************************************/
package org.kalypso.contribs.eclipse.ui.ide.application;
...
/**
 * The idle helper detects when the system is idle in order to perform garbage collection in a way that minimizes impact
 * on responsiveness of the UI. The algorithm for determining when to perform a garbage collection is as follows: -
 * Never gc if there is a test harness present - Don't gc if background jobs are running - Don't gc if the keyboard or
 * mouse have been active within IDLE_INTERVAL - Don't gc if there has been a GC within the minimum gc interval (system
 * property PROP_GC_INTERVAL) - After a gc, don't gc again until (duration * GC_DELAY_MULTIPLIER) has elapsed. For
 * example, if a GC takes 100ms and the multiplier is 60, don't gc for at least five seconds - Never gc again if any
 * single gc takes longer than system property PROP_GC_MAX
 */
class IDEIdleHelper
{
  ...
  /**
   * The default maximum duration for a garbage collection, beyond which the explicit gc mechanism is automatically
   * disabled.
   */
  ...
  /**
   * The name of the boolean system property that specifies whether explicit garbage collection is enabled.
   */
  private static final String PROP_GC = "ide.gc"; //$NON-NLS-1$

  /**
   * The name of the integer system property that specifies the minimum time interval in milliseconds between garbage
   * collections.
   */
  private static final String PROP_GC_INTERVAL = "ide.gc.interval"; //$NON-NLS-1$

  /**
   * The name of the integer system property that specifies the maximum duration for a garbage collection. If this
   * duration is ever exceeded, the explicit gc mechanism is disabled for the remainder of the session.
   */
  ...
  /**
   * The maximum gc duration. If this value is exceeded, the entire explicit gc mechanism is disabled.
   */
