...
public class ExcelLikeTableCursor extends TableCursor
{
  ...
  void addToSelection( final TableItem item )
  {
    if( DEBUG )
  ...
  void toggleSelection( final TableItem item )
  {
    ...
    final List<Object> itemList = new ArrayList<Object>( Arrays.asList( viewerSelection ) );
    if( itemList.contains( item ) )
      itemList.remove( item );
    else
      itemList.add( item );
    
    final TableItem[] newViewerSelection = itemList.toArray( new TableItem[itemList.size()] );
    m_viewer.setSelection( newSelection, true );
    m_viewer.getTable().setSelection( newViewerSelection );
  }
