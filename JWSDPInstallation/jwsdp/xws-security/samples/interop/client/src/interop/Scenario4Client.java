...
/*
 * Implementation of interop scenario # 4 client.
 * 
 * @author Kumar Jayanti (vbkumar.jayanti@sun.com)
 */
public class Scenario4Client {
    ...
    private static String sessionKeyFile =
        System.getProperty("sessionkey.url");
    ...
    private static QName portName =
        new QName("http://xmlsoap.org/Ping", "Ping4");

    private static void scenario4(PingService service) throws Exception {
        try {
    ...
    public static void main(String[] args) throws Exception {
            ...
            String serviceURLFragment = System.getProperty("scenario4.service.url");        
            String serviceURL =
            ...
            scenario4(pingService);
            // use static stubs to override endpoint property of WSDL
            PingPort_Ping4_Stub stub =
                (PingPort_Ping4_Stub) (pingService.getPing4());

            stub._setProperty(
                javax.xml.rpc.Stub.ENDPOINT_ADDRESS_PROPERTY, serviceURL);

            stub.ping(new TicketType("Id", "SUNW"), "Sun Microsystems says hello scenario 4!");
    ...
    protected static SecurityEnvironment initializeSecurityEnvironment()
        ...
        // ------------------ Mutual Agreements ---------------------
        // CertValue in Scenario 4 is receiver's certificate
        ...
        // set the agreement CERT-VALUE for scenario 4
        domain.setAgreementProperty("CERT-VALUE", certValue);

        // read the session key from file
        File f = new File(sessionKeyFile);
