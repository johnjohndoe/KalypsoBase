 ...
 */
public final class TabItemMoveForwardAction extends Action implements ITabAction
{
  private final TabViewer m_tabViewer;

  public TabItemMoveForwardAction( final TabViewer tabViewer )
  {
  ...
  public void update( final CTabFolder folder )
  {
    ...
    final int index = m_tabViewer.getTabFolder().getSelectionIndex();
    final ITabItem[] items = list.getItems();
    setEnabled( items.length > 1 && index < items.length - 1 );
  }
}