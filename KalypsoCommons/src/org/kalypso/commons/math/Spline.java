...
public class Spline
{
   ...
   */
  public Spline( final double[] x, final double[] y )
  {
    setPoints( x, y );
  }
  ...
  public double eval( final double value )
  {
    ...
    final double t = (value - m_x[i]) / m_u[i];

    return t * m_y[i + 1] + (1 - t) * m_y[i] + m_u[i] * m_u[i] * (f( t ) * m_p[i + 1] + f( 1 - t ) * m_p[i]) / 6.0;
  }

  private double f( final double d )
  {
    return d * d * d - d;
  }
  ...
  private void makeSpline( )
  {
    ...
    for( int i = 1; i < N - 1; i++ )
      w[i] = 6.0 * ((m_y[i + 1] - m_y[i]) / m_u[i] - (m_y[i] - m_y[i - 1]) / m_u[i - 1]);
