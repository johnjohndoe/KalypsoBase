 ...
 *  http://ibpm.bjoernsen.de/kalypso/
 *
 *  Copyright (C) 2006 by:
 * 
 ...
 *  ======================================================
	...
	xsl transformation to prepare ogc schemas for jax-binding,
	generates binding customization into schemas to resolve conflicts
-->
<stylesheet xmlns="http://www.w3.org/1999/XSL/Transform"
	extension-element-prefixes="gml"
	xmlns:gml="http://www.opengis.net/gml"
	xmlns:xsd="http://www.w3.org/2001/XMLSchema"
	xmlns:jxb="http://java.sun.com/xml/ns/jaxb" version="1.0">
	<output method='xml' indent='yes' media-type="text/xml"
		encoding="UTF-8" />

	<!--
		tricky: copy root element including all namespace declarations, 
		problem was: if you look from a xml perspective some namespace declarations are not 
		used and so xsl will not generate them into the target document, but
		if you look from a xml-schema perspective they are really essential and must not get lost.
	-->
				...
				<choose>
					...
					<otherwise>
						<variable name="duplicate">
							...
							<value-of
								select="translate(substring($className,1,1),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')" />
							<value-of select="substring($className,2)" />
						</variable>
						<choose>
							<when
 								test="/xsd:schema/xsd:element/@name=$duplicate or /xsd:schema/xsd:complexType/@name=$duplicate">
								<attribute name="name">
									<value-of select="$className" />2</attribute>
							</when>
							<otherwise>
