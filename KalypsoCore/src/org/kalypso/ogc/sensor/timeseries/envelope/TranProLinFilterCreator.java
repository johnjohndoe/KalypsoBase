...
public class TranProLinFilterCreator implements IFilterCreator
{
  ...
  public IObservationFilter createFilter( final AbstractFilterType aft, final IObservation baseObs, final URL context ) throws SensorException
  {
    ...
    if( axisTypes == null || axisTypes.length() == 0 || axisTypes.indexOf( '*' ) > -1 )
      axisTypes = null;
    final TranProLinFilter filter = new TranProLinFilter( ft.getDateBegin().getTime(), ft.getDateEnd().getTime(), ft.getOperator(), ft.getOperandBegin().doubleValue(), ft.getOperandEnd().doubleValue(), ft.getStatusToMerge(), axisTypes );
