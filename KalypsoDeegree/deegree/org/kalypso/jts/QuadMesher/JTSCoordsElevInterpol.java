...
public class JTSCoordsElevInterpol
{
  ...
  public Coordinate[][] calculateElevations( )
  {
    ...
    {
      calculateElevationsForColumn( j );
    }

    return m_coords;
  }

  public void calculateElevationsForColumn( final int column )
  {
    final int numLong = m_coords[column].length;

    final double zStart = m_coords[column][0].z;
    final double zEnd = m_coords[column][numLong - 1].z;

    // calculate the route along the coordinates
    final double distTotal = calcRoute( column, 0, numLong );
    ...
    for( int i = 1; i < numLong - 1; i++ )
    {
      ...
      dist = dist + m_coords[column][i].distance( m_coords[column][i - 1] );
      ...
      m_coords[column][i].z = zStart * (1 - ratio) + zEnd * ratio;
    }
  }
