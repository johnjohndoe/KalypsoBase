...
/**
 * A content handler for gml:MultiCurve
 *
 * @author Felipe Maximino
 */
public class MultiCurveContentHandler extends GMLElementContentHandler implements ICurveHandler
{
  ...
  private GM_MultiCurve m_multiCurve;

  private String m_activeSrs;

  public MultiCurveContentHandler( final XMLReader reader, final UnmarshallResultEater resultEater, final IGmlContentHandler parentContenthandler, final String defaultSrs )
  {
    this( reader, resultEater, parentContenthandler, defaultSrs, ELEMENT_MULTI_CURVE );
  }

  protected MultiCurveContentHandler( final XMLReader reader, final UnmarshallResultEater resultEater, final IGmlContentHandler parentContenthandler, final String defaultSrs, final String elementName )
  {
  ...
  {
    final GM_MultiCurve curve = endMultiCurve();
  ...
  protected void doStartElement( final String uri, final String localName, final String name, final Attributes atts )
  {
    ...
    final ICurveHandler memberHandler = createMemberHandler( m_activeSrs );
    memberHandler.activate();
  }

  protected ICurveHandler createMemberHandler( final String activeSrs )
  {
    return new CurveMemberContentHandler( getXMLReader(), this, activeSrs );
  }
  ...
  {
    return GM_MultiCurve.MEMBER_CURVE.getLocalPart();
  }
  ...
  {
    m_curves.add( element );
  }

  private GM_MultiCurve endMultiCurve( )
  {
    final GM_Curve[] lineStringsArr = m_curves.toArray( new GM_Curve[m_curves.size()] );
    m_multiCurve = GeometryFactory.createGM_MultiCurve( lineStringsArr, m_activeSrs );
