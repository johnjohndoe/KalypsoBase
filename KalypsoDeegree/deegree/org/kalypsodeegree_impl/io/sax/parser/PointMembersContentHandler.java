...
/**
 * A content handler that parses a gml:pointMembers property.<br>
 *
...
public class PointMembersContentHandler extends GMLElementContentHandler implements IPointHandler
{
  ...
  public void handleUnexpectedStartElement( final String uri, final String localName, final String name, final Attributes atts ) throws SAXException
  {
    ...
    if( uri.equals( GM_Point.POINT_ELEMENT.getNamespaceURI() ) && localName.equals( GM_Point.POINT_ELEMENT.getLocalPart() ) )
    {
      final PointContentHandler delegate = new PointContentHandler( getXMLReader(), this, getDefaultSrs() );
      delegate.activate();
      delegate.startElement( uri, localName, name, atts );
    }
    else
      throwSAXParseException( "Unexpected start element: {%s}%s = %s - should be {%s}%s", uri, localName, name, NS.GML3, ELEMENT_POINT_MEMBERS );
  }
