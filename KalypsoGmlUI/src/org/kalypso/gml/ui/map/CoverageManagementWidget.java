 ...
 * collection. In addition he can jump to the extend of an collection element in the map.
 * 
 * @author Thomas Jung
 */
public class CoverageManagementWidget extends AbstractWidget implements IWidgetWithOptions
{
  private static final IKalypsoThemePredicate COVERAGE_PREDICATE = new IKalypsoThemePredicate()
  {
    public boolean decide( IKalypsoTheme theme )
    {
      ...
      IRelationType targetPropertyType = featureList.getParentFeatureTypeProperty();
      IFeatureType targetFeatureType = targetPropertyType.getTargetFeatureType();

      return GMLSchemaUtilities.substitutes( targetFeatureType, ICoverage.QNAME );
      // return GMLSchemaUtilities.substitutes( coveragesFeature.getFeatureType(), ICoverageCollection.QNAME );
    }
  };
  ...
  private Action[] m_customActions;
   ...
   *            The tooltip of the widget.
   */
  public CoverageManagementWidget( final String name, final String tooltip, Action[] customActions )
  {
    super( name, tooltip );
  ...
  protected void handleGenerateColorMap( final Event event )
  {
    ...
    if( input != null )
    {
      ...
      getMapPanel().getMapModell().invalidate( null );
    }
  }
  ...
  private void saveStyle( )
  {
    ...
    final ICoreRunnableWithProgress operation = new ICoreRunnableWithProgress()
    {
      public IStatus execute( IProgressMonitor monitor ) throws CoreException
      {
        ...
        for( UserStyle userStyle : styles )
        {
          if( userStyle instanceof GisTemplateUserStyle )
          {
            GisTemplateUserStyle style = (GisTemplateUserStyle) userStyle;
  ...
  private void initalizeCoverageActions( final FormToolkit toolkit, final Composite parent, final Action[] customActions )
  {
    ...
    for( int i = 0; i < customActions.length; i++ )
      createButton( toolkit, parent, customActions[i] );
  }
  ...
  protected void handleCoverageAdded( final Event event )
  {
    ...
    // TODO: what to do if gridFolder is null? Maybe ask user for target folder?
    Assert.isNotNull( m_gridFolder );

    final RectifiedGridCoverageImportWizard wizard = new RectifiedGridCoverageImportWizard();
    wizard.setCoverageCollection( coverages );
    wizard.init( PlatformUI.getWorkbench(), null );
    wizard.setGridFolder( m_gridFolder );

    final WizardDialog wizardDialog = new WizardDialog( shell, wizard );
    if( wizardDialog.open() != Window.OK )
      return;

    final GM_Envelope scaledBox = GeometryUtilities.scaleEnvelope( wizard.getNewCoverage().getEnvelope(), 1.05 );
    getMapPanel().setBoundingBox( scaledBox );
    ...
    final ICoreRunnableWithProgress operation = new ICoreRunnableWithProgress()
    {
      public IStatus execute( IProgressMonitor monitor ) throws InvocationTargetException
      {
        try
        {
          ...
          final IStatus status = CoverageManagmentHelper.deleteGridFile( selectedCoverage );
          if( !status.isOK() )
            return status;
