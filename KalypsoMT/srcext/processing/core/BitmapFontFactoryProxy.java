...
// FontManager.getInstance().registerFontFactory("", new BitmapFontFactory());
  ...
  @Override
  public IFont getCopy( final IFont font )
  {
    if( font instanceof BitmapFont )
    {
      final BitmapFont bf = (BitmapFont) font;
      final BitmapFont copy = new BitmapFont( (BitmapFontCharacter[]) bf.getCharacters(), bf.getDefaultHorizontalAdvX(), bf.getFontFamily(), bf.getFontMaxAscent(), bf.getFontMaxDescent(), bf.getUnitsPerEM(), bf.getOriginalFontSize(), bf.getFillColor(), /*
                                                                                                                                                                                                                                                              * bf
                                                                                                                                                                                                                                                              * .
                                                                                                                                                                                                                                                              * getStrokeColor
                                                                                                                                                                                                                                                              * (
                                                                                                                                                                                                                                                              * )
                                                                                                                                                                                                                                                              * ,
                                                                                                                                                                                                                                                              */bf.isAntiAliased(), bf.getFontFileName() );
      return copy;
    }
    return null;
  }
  ...
  @Override
  public IFont createFont( final PApplet pa, final String fontName, final int fontSize, final MTColor color )
  {
    return this.createFont( pa, fontName, fontSize, color, color, true );
  }

  @Override
  public IFont createFont( final PApplet pa, final String fontName, final int fontSize, final MTColor color, final boolean antiAliased )
  {
  ...
  /*
   * (non-Javadoc)
   * @see org.mt4j.components.visibleComponents.font.fontFactories.IFontFactory#createFont(processing.core.PApplet,
   * java.lang.String, int, org.mt4j.util.MTColor, org.mt4j.util.MTColor)
   */
  @Override
  public IFont createFont( final PApplet pa, final String fontFileName, final int fontSize, final MTColor fillColor, final MTColor strokeColor )
  {
   ...
   */
  @Override
  public IFont createFont( final PApplet pa, final String fontFileName, final int fontSize, final MTColor fillColor, final MTColor strokeColor, final boolean antiAliased )
  {
    PFont p5Font = null;
    try
    {
    ...
    final List<BitmapFontCharacter> bitMapCharacters = this.createCharacters( pa, p5Font, defaultCharacters, fillColor /*
                                                                                                                        * ,
                                                                                                                        * strokeColor
                                                                                                                        */);
    ...
    /*
     * Font f = p5Font.getFont(); FontMetrics fm = pa.getFontMetrics(f); Map<TextAttribute, ?> atts = f.getAttributes();
     * Set<TextAttribute> attKeys = atts.keySet(); for (Iterator iterator = attKeys.iterator(); iterator.hasNext();) {
     * TextAttribute textAttribute = (TextAttribute) iterator.next(); Object value = atts.get(textAttribute);
     * logger.debug("Key: " + textAttribute + " Value: " + value); } // FontMetrics fm =
     * Toolkit.getDefaultToolkit().getFontMetrics(f);
     */

    final int defaultHorizontalAdvX = (!bitMapCharacters.isEmpty()) ? bitMapCharacters.get( 0 ).getHorizontalDist() : Math.round( p5Font.descent() * fontSize ); // FIXME
// HACK!
    final String fontFamily = p5Font.getPostScriptName();
...
// int fontMaxAscent = p5Font.lazyMetrics.getAscent();
    ...
    /*
     * //TODO INFO: because in vector font this is a negative value, too Font f = p5Font.getFont(); if (f != null){
     * FontMetrics fm = pa.getFontMetrics(f); fontMaxDescent = fm.getDescent(); }
     */
...
// int fontMaxDescent = fm.getMaxDescent();
    final int unitsPerEm = 1000; // FIXME HACK!
    final int originalFontSize = fontSize; // important for font cache

    final PImage dummy = new PImage( 1, 1 );
// /*
    // Manually add a newLine character to the font
    final BitmapFontCharacter newLine = new BitmapFontCharacter( pa, dummy, "\n", 0, 0, 0 );
...
// int spaceAdvancex = p5Font.getGlyph('-').width;
    final int spaceAdvancex = Math.round( (p5Font.width( 'i' ) * fontSize) );
// int spaceAdvancex = Math.round(pa.textWidth(' '));
// int spaceAdvancex = Math.round(p5Font.width(' ') * p5Font.size);
    final BitmapFontCharacter space = new BitmapFontCharacter( pa, dummy, " ", 0, 0, spaceAdvancex );
    ...
    final int defaultTabWidth = spaceAdvancex * 4;
    final BitmapFontCharacter tab = new BitmapFontCharacter( pa, dummy, "\t", 0, 0, defaultTabWidth );
    try
    {
      final int tabWidth = 4 * space.getHorizontalDist();
...
// */
    ...
    // TODO bitmap font size seems different to same size vector font, we must have check descent -> textarea ->
// res*em*etc
    // TODO eureka font - numbers baseline wrong?

    // Create the bitmap font
    final BitmapFontCharacter[] characters = bitMapCharacters.toArray( new BitmapFontCharacter[bitMapCharacters.size()] );
    final BitmapFont bitmapFont = new BitmapFont( characters, defaultHorizontalAdvX, fontFamily, fontMaxAscent, fontMaxDescent, unitsPerEm, originalFontSize, fillColor,
// strokeColor,
    antiAliased, fontFileName );
...
// }
   ...
   */
  private PFont getProcessingFont( final PApplet pa, final String fontFileName, final int fontSize, final boolean antiAliased ) throws FileNotFoundException
  {
    PFont p5Font;
    ...
    if( fontFileName.endsWith( ".vlw" ) )
    {
      ...
      if( p5Font == null )
      {
        final int lastDirFileSeparator = fontFileName.lastIndexOf( java.io.File.separator );
        final int lastDirSeparator = fontFileName.lastIndexOf( AbstractMTApplication.separator );
    ...
    else
    {
      ...
      if( lastDirFileSeparator != -1 )
      {
        p5Font = pa.createFont( fontFileName.substring( lastDirFileSeparator + 1, fontFileName.length() ), fontSize, antiAliased ); // Creats
// the font
      }
      else if( lastDirSeparator != -1 )
      {
        p5Font = pa.createFont( fontFileName.substring( lastDirSeparator + 1, fontFileName.length() ), fontSize, antiAliased ); // Creats
// the font
      }
      else
      {
  ...
  public List<BitmapFontCharacter> getCharacters( final PApplet pa, final String chars, final MTColor fillColor,
// MTColor strokeColor,
  final String fontFileName, final int fontSize )
  {
  ...
  public List<BitmapFontCharacter> getCharacters( final PApplet pa, final String chars, final MTColor fillColor,
// MTColor strokeColor,
  final String fontFileName, final int fontSize, final boolean antiAliased )
  {
    PFont p5Font = null;
    try
    {
  ...
  private List<BitmapFontCharacter> createCharacters( final PApplet pa, final PFont p5Font, final String chars, final MTColor fillColor /*
                                                                                                                                         * ,
                                                                                                                                         * MTColor
                                                                                                                                         * strokeColor
                                                                                                                                         */)
  {
    final List<BitmapFontCharacter> bitMapCharacters = new ArrayList<BitmapFontCharacter>();

    for( int i = 0; i < chars.length(); i++ )
    {
      final char c = chars.charAt( i );
// int charIndex = p5Font.index(c);
      final Glyph glyph = p5Font.getGlyph( c );
      if( glyph != null )
      {
        PImage charImage = glyph.image;
        final int charWidth = glyph.width;
        final int charHeight = glyph.height;
        final int topExtend = glyph.topExtent;
        int leftExtend = glyph.leftExtent;
        final int widthDisplacement = glyph.setWidth;
...
// }
        ...
        final int topShiftAmount = 1;
        final int leftShiftAmount = 1;

// PImage copy = new PImage(ToolsMath.nearestPowerOfTwo(charWidth + shiftAmount), ToolsMath.nearestPowerOfTwo(charHeight
// + shiftAmount), PImage.ARGB);
//
        final PImage copy = new PImage( nextPowerOfTwo( charImage.width + leftShiftAmount + 1 ), nextPowerOfTwo( charImage.height + topShiftAmount + 1 ), PImage.ARGB );
...
// copy.copy(charImage, 0, 0, charImage.width, charImage.height, leftShiftAmount, topShiftAmount, charImage.width,
// charImage.height);

// copy.copy(charImage, 0, 0, charWidth, charHeight, shiftAmount, shiftAmount, charWidth, charHeight);
// copy.copy(charImage, 0, 0, charImage.width, charImage.height, shiftAmount, shiftAmount, charImage.width,
// charImage.height);
// copy.copy(charImage, 0, 0, charImage.width, charImage.height, shiftAmount, shiftAmount, charImage.width,
// charImage.height);
// copy.copy(charImage, 0, 0, charWidth, charHeight, shiftAmount, shiftAmount, charWidth, charHeight);
...
// }
        ...
        final String StringChar = Character.toString( c );
        final BitmapFontCharacter character = new BitmapFontCharacter( pa, charImage, StringChar, leftExtend, topOffset, widthDisplacement );
        ...
        // logger.debug("Char: " + c + " charWidth: " + charWidth + " leftExtend: " + leftExtend +
// " widthDisplacement: " + widthDisplacement + " imageHeight: " + charImage.height + " charHeight: " + charHeight +
// " topExtent: " + topExtend);
      }
      else
      {
  ...
  private int nextPowerOfTwo( final int val )
  {
