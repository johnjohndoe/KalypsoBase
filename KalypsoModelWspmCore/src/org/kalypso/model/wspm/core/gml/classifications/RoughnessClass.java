...
public class RoughnessClass extends AbstractClassificationClass implements IRoughnessClass
{
  ...
  {
    final String ks = formatValue( "ks=", "%.3f", getKsValue() ); //$NON-NLS-1$ //$NON-NLS-2$
    final String kst = formatValue( "kst=", "%.0f", getKstValue() ); //$NON-NLS-1$ //$NON-NLS-2$

    return String.format( "%s (%s, %s)", getDescription(), ks, kst ); //$NON-NLS-1$
  }
}