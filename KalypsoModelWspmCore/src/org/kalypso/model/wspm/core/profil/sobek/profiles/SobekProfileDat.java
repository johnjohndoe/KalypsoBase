...
public class SobekProfileDat
{
  ...
  public String serialize( )
  {
    ...
    /* Build the line. */
    line.append( String.format( Locale.PRC, "CRSN id '%s' di '%s' rl %s ", m_id, m_di, m_rl ) ); //$NON-NLS-1$

    if( m_ll != null )
      line.append( String.format( Locale.PRC, "ll %s ", m_ll ) ); //$NON-NLS-1$

    if( m_rs != null )
      line.append( String.format( Locale.PRC, "rs %s ", m_rs ) ); //$NON-NLS-1$

    if( m_ls != null )
      line.append( String.format( Locale.PRC, "ls %s ", m_ls ) ); //$NON-NLS-1$
