...
{
  static final double CRD_EPS = 0.10;

  static final double POLY_EPS = 0.01;
  ...
  private int crdIndexOfWithEps( final List<Coordinate> crdList, final Coordinate crd )
  {
    ...
    for( int i = 0; i < crdList.size(); i++ )
    {
      ...
      if( dist < CRD_EPS && dist < bestDisttance )
      {
        bestIndex = i;
        bestDisttance = dist;
      }
  ...
  private void liftLower( final CoordinateSequence lowerSeq, final Coordinate[] upperCrdArray )
  {
    ...
    final PolyLine polyLine = new PolyLine( lowerXs, lowerYs, POLY_EPS );
    ...
    for( final Coordinate upperCrd : upperCrdArray )
    {
      ...
      if( diff > -POLY_EPS )
      {
        ...
        else if( diff > POLY_EPS )
        {
          ...
          upperCrd.y = lowerY + POLY_EPS;
        }
        else
        {
          addStatus( IStatus.INFO, String.format( "Tiny adaption of upper coordinate by: %.2f [m]", Math.abs( diff + POLY_EPS ) ), null ); //$NON-NLS-1$
