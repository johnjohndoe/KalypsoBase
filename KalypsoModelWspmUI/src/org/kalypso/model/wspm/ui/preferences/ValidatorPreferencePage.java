...
public class ValidatorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage
{
  ...
  public ValidatorPreferencePage( )
  {
    ...
    setPreferenceStore( KalypsoModelWspmCorePlugin.getDefault().getPreferenceStore() );

    setDescription( Messages.getString( "org.kalypso.model.wspm.ui.preferences.ValidatorPreferencePage.0" ) ); //$NON-NLS-1$
  }

  @Override
  public void createFieldEditors( )
  {
    addField( new BooleanFieldEditor( WspmCorePreferences.P_VALIDATE_PROFILE, Messages.getString( "org.kalypso.model.wspm.ui.preferences.ValidatorPreferencePage.1" ), getFieldEditorParent() ) ); //$NON-NLS-1$

    final Object[] elements = KalypsoModelWspmCorePlugin.getDefault().getValidatorFactory().getAllRules();
    addField( new ChecklistFieldEditor( elements, "getID", new ValidatorLabelProvider(), WspmCorePreferences.P_VALIDATE_RULES_TO_EXCLUDE, "", getFieldEditorParent() ) ); //$NON-NLS-1$ //$NON-NLS-2$
  }
