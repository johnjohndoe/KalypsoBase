...
public class ValidateProfilesWizard extends Wizard implements IWorkbenchWizard
{
  ...
  {
   final IProfileFeature[] profileFeatures = ValidateProfileHelper.toProfiles( m_profileChooserPage.getChoosen() );
    final IValidatorRule[] rules = ValidateProfileHelper.toRules( m_validatorChooserPage.getChoosen() );
    final Object[] quickFixes = m_quickFixChoosePage.getChoosen();

    final ValidateProfilesJob job = new ValidateProfilesJob( profileFeatures, rules, quickFixes, m_workspace );
    job.schedule();
