...
public class MarkerInfoAction extends Action
{
  ...
  public MarkerInfoAction( final IMarker marker, final IProfile profil )
  {
    ...
    setText( marker.getAttribute( IMarker.MESSAGE, StringUtils.EMPTY ) );
    setImageDescriptor( getImageDescriptor( marker ) );
  }
  ...
  public void run( )
  {
    ...
    final int pointPos = m_marker.getAttribute( IValidatorMarkerCollector.MARKER_ATTRIBUTE_POINTPOS, -1 );
    if( pointPos < 0 )
      return;

    final IProfileRecord record = m_profil.getPoint( pointPos );
    m_profil.getSelection().setActivePoints( record );
  }

  // TODO: move into helper
  private static ImageDescriptor getImageDescriptor( final IMarker marker )
  {
