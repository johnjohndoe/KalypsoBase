...
public class TranscendenceProjectRowBuilder extends AbstractLocalProjectRowBuilder
{
  ...
  public TranscendenceProjectRowBuilder( final ITranscendenceProject transcendence, final IKalypsoModule module, final IProjectDatabaseUiLocker locker )
  {
    super( transcendence, module, locker );
  ...
  public void render( final Composite parent, final FormToolkit toolkit )
  {
    try
    {
      ...
      if( preferences.isLocked() )
      {
        ...
        lnk.setToolTipText( String.format( "÷ffne Projekt: %s (Bearbeitungsmodus)", getLocalProject().getName() ) ); 
      }
      else if( !ProjectDatabaseServerUtils.isServerOnline() )
      {
        lnk.setImage( IMG_LORE_PROJECT_DISABLED );
        lnk.setToolTipText( String.format( "÷ffne Projekt: %s (Deaktiviert - Lesemodus)", getLocalProject().getName() ) ); 
      }
      else if( m_transcendence.getBean().isProjectLockedForEditing() )
      {
        lnk.setImage( IMG_LORE_OTHER_LOCK );
        lnk.setToolTipText( String.format( "÷ffne Projekt: %s (Extern gesperrt - Lesemodus)", getLocalProject().getName() ) ); 
      }
      else
      {
        lnk.setImage( IMG_LORE_PROJECT );
        lnk.setToolTipText( String.format( "÷ffne Projekt: %s (Lesemodus)", getLocalProject().getName() ) ); 
      }
