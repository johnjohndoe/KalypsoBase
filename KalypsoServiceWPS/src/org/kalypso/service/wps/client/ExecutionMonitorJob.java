...
public class ExecutionMonitorJob extends Job
{
  ...
  public ExecutionMonitorJob( final String statusLocation, final ParameterizedCommand command )
  {
    super( "" ); //$NON-NLS-1$
  ...
  protected IStatus run( IProgressMonitor monitor )
  {
    ...
    try
    {
      canceledLoop: while( !monitor.isCanceled() )
      {
        ...
        ExecuteResponseType executeResponse = null;
        try
        {
          ...
          statusFile = VFSUtilities.checkProxyFor( getStatusLocation(), m_fileSystemManager );
          if( statusFile.exists() )
          {
            ...
            if( xml != null && !xml.isEmpty() )
            {
              ...
              executeResponse = executeState.getValue();
            }
          }
        }
        ...
        if( executeResponse != null )
        {
          ...
          final StatusType state = executeResponse.getStatus();
          ...
          // update status and status location
          setStatus( status );
          monitor.subTask( status.getMessage() );
          setStatusLocation( executeResponse.getStatusLocation() );
        }
