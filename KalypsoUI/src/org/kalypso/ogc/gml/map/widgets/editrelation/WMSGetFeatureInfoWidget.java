...
public class WMSGetFeatureInfoWidget extends AbstractWidget implements IWidgetWithOptions
{
  ...
  {
    super( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.1" ), "" ); //$NON-NLS-1$ //$NON-NLS-2$
  }
...
// TODO: check if this repaint is necessary for the widget
  ...
  private void updateInfoText( )
  {
    ...
    if( activeTheme != null )
      ...
      themeName = Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.3" ); //$NON-NLS-1$
    ...
    final IGetFeatureInfoResultProcessor processor = new IGetFeatureInfoResultProcessor()
    {
      ...
      public void write( final String message )
      {
        ...
        try
        {
          ...
          pw.print( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.4" ) + themeName + "\n" ); //$NON-NLS-1$ //$NON-NLS-2$
          pw.print( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.6" ) ); //$NON-NLS-1$
          if( pointOfInterest == null )
            pw.print( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.7" ) ); //$NON-NLS-1$
          else
          {
            pw.print( "\n  " ); //$NON-NLS-1$
          ...
          pw.print( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.10" ) ); //$NON-NLS-1$
          pw.print( "\n " + message ); //$NON-NLS-1$
        }
    ...
    if( pointOfInterest == null )
      processor.write( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.12" ) ); //$NON-NLS-1$
    else if( !(activeTheme instanceof KalypsoWMSTheme) )
      processor.write( Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.13" ) ); //$NON-NLS-1$
    else
    {
  ...
  public Control createControl( final Composite parent, final FormToolkit toolkit )
  {
    ...
    m_textInfo = toolkit.createText( m_topLevel, Messages.get( "org.kalypso.ogc.gml.map.widgets.editrelation.WMSGetFeatureInfoWidget.15" ), SWT.READ_ONLY | SWT.MULTI | SWT.WRAP ); //$NON-NLS-1$
