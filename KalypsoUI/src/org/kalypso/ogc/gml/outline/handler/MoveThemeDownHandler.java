 ...
 */
public class MoveThemeDownHandler extends AbstractHandler
{
  /**
  ...
  public Object execute( final ExecutionEvent event ) throws ExecutionException
  {
    ...
    /* Do nothing if we have selected the last theme */
    final IKalypsoTheme lastTheme = selectedThemesInOrder[selectedThemesInOrder.length - 1];
    final IMapModell lastModell = lastTheme.getMapModell();
    final IKalypsoTheme[] lastModellThemes = lastModell.getAllThemes();
    if( lastModellThemes[lastModellThemes.length - 1] == lastTheme )
      return null;
    ...
    for( int i = selectedThemesInOrder.length; i > 0; i-- )
    {
      ...
      compositeCommand.addCommand( new MoveThemeDownCommand( themeMapModell, kalypsoTheme ) );
    }
