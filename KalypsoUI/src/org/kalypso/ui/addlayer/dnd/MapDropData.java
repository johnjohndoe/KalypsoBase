...
public class MapDropData
{
  ...
  public MapDropData( final String path )
  {
    final URI pathUri = new File( path ).toURI();
    final IFile[] files = ResourcesPlugin.getWorkspace().getRoot().findFilesForLocationURI( pathUri );
    if( files.length == 0 )
      m_path = path;
    else
      m_path = files[0].getFullPath().toString();
  }
