...
public final class ZmlStyleResolver
{
  ...
  {
    if( INSTANCE == null )
      INSTANCE = new ZmlStyleResolver();

    return INSTANCE;
  }

  public CellStyle findStyle( final StyleReferenceType reference ) throws CoreException
  {
    try
    {
      if( reference == null )
  ...
  private CellStyle findUrlStyle( final String uri, final String identifier ) throws MalformedURLException, JAXBException
  {
    ...
    if( styleSet == null )
    {
      ...
      styleSet = tableType.getStyleSet();
      m_styleSetCache.put( uri, styleSet );
    }

    if( styleSet == null )
      return null;

    for( final CellStyleType style : styleSet.getStyle() )
    {
      if( style.getId().equals( identifier ) )
