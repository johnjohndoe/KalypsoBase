 ...
 * @author Dirk Kuch
 */
public class ZmlCombinedChartLegendRenderer implements IChartLegendRenderer
{
  private static final String ID = "org.kalypso.zml.ui.chart.legend.renderer.combined"; //$NON-NLS-1$
  ...
  {
    m_lastRenderer = new ZmlCompactChartLegendRenderer();
    final Point size = m_lastRenderer.calculateSize( canvas, config );
    if( m_lastRenderer.rowSize() == 1 )
      return size;

    m_lastRenderer = new ZmlBlockChartLegendRenderer();
