...
public class ExtendedAxisRenderer extends AbstractGenericAxisRenderer
{
  ...
  private final GenericChartLabelRenderer m_tickRenderer;
  ...
  public ExtendedAxisRenderer( final String id, final int tickLength, final Insets tickLabelInsets, final Insets labelInsets, final int gap, final ILabelCreator labelCreator, final ITickCalculator tickCalculator, final Number minTickInterval, final boolean hideCut, final int fixedWidth, final ILineStyle axisLineStyle, final ITextStyle labelStyle, final ILineStyle tickLineStyle, final ITextStyle tickLabelStyle, final int borderSize, final ALIGNMENT labelPosition )
  {
    ...
    final TitleTypeBean ttb = new TitleTypeBean( "" );
    ttb.setInsets( getTickLabelInsets() );
    ttb.setPositionHorizontal( ALIGNMENT.CENTER );
    ttb.setPositionVertical( ALIGNMENT.CENTER );
    tickLabelStyle.setAlignment( ALIGNMENT.CENTER );
    ttb.setTextStyle( tickLabelStyle );
    m_tickRenderer = new GenericChartLabelRenderer( ttb );
  }
...
// drawText( gc, label, textX, textY, tickLabelStyle );
        testDrawText( gc, label, x2, y2, axis.getPosition() );
      }
    }
  }

  private final void testDrawText( final GC gc, final String label, final int x, final int y, final POSITION position )
  {
    m_tickRenderer.getTitleTypeBean().setLabel( label );
    m_tickRenderer.getTitleTypeBean().setTextAnchorX( position.getOrientation() == ORIENTATION.HORIZONTAL?ALIGNMENT.CENTER:position==POSITION.LEFT?ALIGNMENT.RIGHT:ALIGNMENT.LEFT );
    m_tickRenderer.getTitleTypeBean().setTextAnchorY( position.getOrientation() == ORIENTATION.VERTICAL?ALIGNMENT.CENTER:position==POSITION.TOP?ALIGNMENT.BOTTOM:ALIGNMENT.TOP);
    m_tickRenderer.paint( gc, new Point( x, y ) );
  }
