...
// else
  ...
  public static void addLayer( final IChartModel model, final IReferenceResolver rr, final URL context, final LayerType layerType, final IExtensionLoader extLoader )
  {
    ...
    final MapperType domainAxisType = (MapperType) rr.resolveReference( mapper.getDomainAxisRef().getRef() );
    if( domainAxisType instanceof AxisType )
      addAxis( model, rr, (AxisType) domainAxisType, extLoader, context );
    else if( domainAxisType instanceof ScreenAxisType )
      addScreenAxis( model, (ScreenAxisType) domainAxisType, extLoader );

    final MapperType targetAxisType = (MapperType) rr.resolveReference( mapper.getTargetAxisRef().getRef() );
    if( targetAxisType instanceof AxisType )
      addAxis( model, rr, (AxisType) targetAxisType, extLoader, context );
    else if( targetAxisType instanceof ScreenAxisType )
      addScreenAxis( model, (ScreenAxisType) targetAxisType, extLoader );
