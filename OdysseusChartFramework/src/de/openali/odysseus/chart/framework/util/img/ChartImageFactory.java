...
/**
 * @author burtscher Creates an Image from a chart widget. The objects has to be
 *         disposed when it's no longer needed
 */
public class ChartImageFactory
{
	...
	public static ImageData createChartImage(ChartComposite chart, Device dev, int width, int height)
	{
		...
		for (final Entry<IAxis, IAxisComponent> ac : acs)
		{
			...
			/*
			 * ...und ins Endbild kopieren; dabei muss berï¿½cksichtigt werden,
			 * dass die Components in 2 Ebenen Tiefe liegen - zur Position muss
			 * noch die der Eltern addiert werden
			 */
			gcw.drawImage(tmpImg, comp.getBounds().x, comp.getBounds().y, comp.getBounds().width, comp.getBounds().height, comp.getParent().getBounds().x + comp.getBounds().x, comp.getParent().getBounds().y + comp.getBounds().y, comp.getBounds().width, comp.getBounds().height);
		}
		gcw.dispose();
		tmpGc.dispose();
		tmpImg.dispose();
