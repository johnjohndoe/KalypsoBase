...
{
  private final List<TitleTypeBean> m_titleTypes = new  ArrayList<TitleTypeBean>();
...
//  private int m_centerHeight;
  ...
  public final void addTitle( final TitleTypeBean... titles )
  {
    ...
    {
      m_titleTypes.add( title );
//      final String key = title.getTextAnchorX().name() + title.getTextAnchorY().name();
//      final List<TitleTypeBean> titleList;
//      if( m_titleTypes.get( key ) == null )
//      {
//        titleList = new ArrayList<TitleTypeBean>();
...
//  private Point calcSize( )
//  {
//    m_leftTop = calcSize( m_titleTypes.get( ALIGNMENT.LEFT.name() + ALIGNMENT.TOP.name() ) );
//    m_leftBottom = calcSize( m_titleTypes.get( ALIGNMENT.LEFT.name() + ALIGNMENT.BOTTOM.name() ) );
//    m_leftCenter = calcSize( m_titleTypes.get( ALIGNMENT.LEFT.name() + ALIGNMENT.CENTER.name() ) );
//    m_centerTop = calcSize( m_titleTypes.get( ALIGNMENT.CENTER.name() + ALIGNMENT.TOP.name() ) );
//    m_centerBottom = calcSize( m_titleTypes.get( ALIGNMENT.CENTER.name() + ALIGNMENT.BOTTOM.name() ) );
//    m_rightTop = calcSize( m_titleTypes.get( ALIGNMENT.RIGHT.name() + ALIGNMENT.TOP.name() ) );
//    m_rightBottom = calcSize( m_titleTypes.get( ALIGNMENT.RIGHT.name() + ALIGNMENT.BOTTOM.name() ) );
//    m_rightCenter = calcSize( m_titleTypes.get( ALIGNMENT.RIGHT.name() + ALIGNMENT.CENTER.name() ) );
//    m_centerCenter = calcSize( m_titleTypes.get( ALIGNMENT.CENTER.name() + ALIGNMENT.CENTER.name() ) );
//
...
//  }
  ...
  public void paint( final GC gc, final Rectangle boundsRect )
  {
   // int lineTop = 0;
    ...
    {
      m_renderer.setTitleTypeBean( titleType );
      m_renderer.paint( gc,boundsRect);//new Rectangle( boundsRect.x, boundsRect.y+lineTop, boundsRect.width, boundsRect.height ) );
   //   lineTop += m_renderer.getSize().y;
    }
...
//  private void paintTitle( final GC gcw, final List<TitleTypeBean> titleTypes, final Point size, final int left, final int top, final int width, final int height )
//  {
//    if( titleTypes == null )
//      return;
//    final IChartLabelRenderer renderer = new GenericChartLabelRenderer();
//    int centerLinePos = (height - size.y) / 2;
//    for( final TitleTypeBean titleType : titleTypes )
//    {
//      renderer.setTitleTypeBean( titleType );
