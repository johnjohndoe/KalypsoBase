...
public class PlotCanvas extends Canvas implements PaintListener, Listener
{
	 ...
	 * 
	 * @param layers
	 *            Array of layers which should be redrawn; if null, only the
	 *            bufferImage is redrawn
	 */
	 ...
	 *      Draws all layers if a PaintEvent is thrown; only exeption: if the
	 *      PaintEvent is thrown by a MouseDrag-Action, a buffered plot image is
	 *      used
	 */
	...
	private void paintEditInfo(final GC gcw)
	{
		if (m_editInfo != null)
		{
			...
			if (tooltiptext != null && mousePos != null)
			{
				...
				int toolx = mousePos.x + 20 + TOOLINSET;
	...
	/**
	 * double-buffered paint method; set to public in order to be used from
	 * ouside, e.g. from ChartImageContainer
	 */
	...
	private Image paintBufferedLayer(GC gcw, Rectangle screen, IChartLayer layer, Image layerImage)
	{
		...
		if (layerImage == null)
		{
			...
			Color transparentColor = new Color(gcw.getDevice(), new RGB(0xfe, 0xff, 0xff));
			...
			// Hintergrund explizit malen - der wird sp�ter transparent
			// gezeichnet
	...
	/**
	 * sets an offset to which the paint buffer is moved in case of a pan
	 * action;
	 * 
	 * @param layers
	 *            if null, all layers are moved; else, only mentioned layers are
	 *            moved
	 * 
	 * @param offset
	 *            positive value moves buffer to right / down, negative value to
	 *            left / up
	 */
