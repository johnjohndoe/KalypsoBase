...
public class PlotCanvas extends Canvas implements PaintListener, Listener
{
  ...
  private void paintDragArea( final GC gcw )
  {
    ...
    if( m_dragArea != null )
    {
      ...
      // TODO: SWT-Bug mit drawFocus (wird nicht immer gezeichnet), irgendwann mal wieder �berpr�fen
      gcw.setAlpha( 50 );
  ...
  private void paintTooltip( final GC gcw )
  {
    // Tooltips zeichnen
    if( m_hoverInfo != null )
    {
  ...
  private Image paintBufferedLayer( GC gcw, Rectangle screen, IChartLayer< ? , ? > layer, Image layerImage )
  {
    ...
    if( layerImage == null )
    {
      ...
      // Hintergrund explizit malen - der wird sp�ter transparent gezeichnet
      ...
      try
      {
        ...
        try
        {
          ...
          System.out.println( "Time to paint layer (" + layer.getTitle() + "): " + (end - start) + " ms" );
        }
        catch( final SWTException e )
        {
          e.printStackTrace();
