...
public class ImageOutput
{
	...
	public static void imageResponse(RequestBean request,
	        ResponseBean response, ImageData id)
	{
		...
		if ((imgTypeStr != null) && !imgTypeStr.toLowerCase().equals("png"))
			...
			else if (imgTypeStr.toLowerCase().equals("gif"))
			{
				...
				if (!id.palette.isDirect)
				{
					...
					if (rgbs != null)
						Logger.logInfo(Logger.TOPIC_LOG_GENERAL,
						        "RGBs hat die Größe " + rgbs.length);
					else
						Logger.logInfo(Logger.TOPIC_LOG_GENERAL,
						        "RGBs ist null ");
				}
				...
				/**
				 * Wenn man den Array über rgbs.toArray() erzeugt, dasnn gibt es
				 * eine trotz cast nach (RGB[]) eine ClassCastException
				 */
				...
				for (int y = 0; y < id2.height; y++)
					for (int x = 0; x < id2.width; x++)
						id2.setPixel(x, y, pd2.getPixel(pd.getRGB(id.getPixel(
						        x, y))));

				id = id2;

				Logger.logInfo(Logger.TOPIC_LOG_GENERAL, "Farbenanzahl: "
				        + pixels.size());
		...
		if (transparency != null)
		{
			if (transparency.toLowerCase().equals("true"))
				;
			{
				id.transparentPixel = id.palette
