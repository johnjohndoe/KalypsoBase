...
public class ODSChartManipulation
{
	...
	public static void manipulateChart(IChartModel model, RequestBean request)
	        throws OWSException
	{
	...
	/**
	 * sets the layers visibility and position from the request parameter LAYERS
	 * - if the parameter is not empty, all those layers which are not mentioned
	 * in the comma separated list are set invisible
	 */
	private static void setLayerVisibility(ILayerManager manager,
	        RequestBean request)
	{
		...
		if ((realReqLayers != null) && (realReqLayers.size() > 0))
		{
			...
			{
				final IChartLayer layer = manager.getLayerById(realReqLayers
				        .get(i));
				if (layer != null)
				{
	...
	private static void setAxesRanges(IMapperRegistry ar, RequestBean request)
	        throws OWSException
	{
		setAxesRanges(ar, request.getParameterValue("AXES"), request
		        .getParameterValue("AXES_MIN"), request
		        .getParameterValue("AXES_MAX"));
	}

	private static void setAxesRanges(IMapperRegistry mr, String axesString,
	        String minsString, String maxsString) throws OWSException
	{
		...
		/*
		 * Zunächst AXISMIN- und AXISMAX-Parameter parsen und zu Min/MaxMap
		 * hinzufügen
		 */
		...
		if ((axesStrArray.length != minStrArray.length)
		        || (axesStrArray.length != maxStrArray.length))
			throw new OWSException(
			        ExceptionCode.INVALID_PARAMETER_VALUE,
			        "AXES, AXES_MIN and AXES_MAX must contain equal number of values",
			        null);
	...
	/**
	 * sets range for an individual axis; set to public so an axis can be
	 * re-ranged widthout the need for a complete chartmodel (e.g. by
	 * GetAxesInfo, GetAxis)
	 * 
	 * @param iaxis
	 ...
	 */
	public static <T> void setAxisRange2(final IAxis iaxis,
	        final Class<T> clazz, String maxString, String minString)
	        throws OWSException
	{
		...
		catch (MalformedValueException e)
		{
			throw new OWSException(
			        ...
			        "Value '" + minString + "' is not appropriate for Axis '"
			                + iaxis.getId() + "'", null);
		}
		try
		{
		...
		catch (MalformedValueException e)
		{
			throw new OWSException(
			        OWSException.ExceptionCode.INVALID_PARAMETER_VALUE,
			        "Value '" + maxString + "' is not appropriate for Axis '"
			                + iaxis.getId() + "'", null);
		}
