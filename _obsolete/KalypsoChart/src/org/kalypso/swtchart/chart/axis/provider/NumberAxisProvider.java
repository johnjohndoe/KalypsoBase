 ...
 *
 */
public class NumberAxisProvider implements IAxisProvider
{
  ...
  public IAxis getAxis( ) throws AxisProviderException
  {
    ...
    IAxis<Number> axis = new NumberAxis( m_at.getName(), m_at.getLabel(), PROPERTY.CONTINUOUS, pos, dir, new DoubleComparator( 0.001 ) );
    if (axis!=null)
    {
      try
      {
        NumberParser np=new NumberParser();
        axis.setFrom( np.createValueFromString(m_at.getMinVal()) );
        axis.setTo( np.createValueFromString(m_at.getMaxVal()) );
      }
      catch( MalformedValueException e )
      {
        // TODO Exception auswerten
        e.printStackTrace();
      }
    }
    else
  ...
  public IAxisRenderer getRenderer( )
  {
    ...
    IAxisRenderer<Number> numberAxisRenderer = new NumberAxisRenderer( fgRGB, bgRGB, 1, 5, insetsTick, 1, insetsLabel, 0, fdLabel, fdTick );
