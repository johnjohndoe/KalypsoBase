...
public class AxisRegistry implements IAxisRegistry
{
  ...
  public void addAxis( IAxis axis )
  {
    ...
    if( m_axes.containsKey( axis.getIdentifier() ) )
      Logger.trace( "Axis already present in registry: " + axis.getIdentifier() + " - " + axis.getLabel() );
  ...
  /**
   * @see org.kalypso.swtchart.chart.axis.registry.IAxisRegistry#logicalToScreen(org.kalypso.swtchart.chart.layer.IChartLayer)
   */
  public<T1, T2> Point logicalToScreen( IChartLayer l, T1 domVal, T2 valVal  )
  {
    IAxis<T1> domAxis=l.getDomainAxis();
    IAxis<T2> valAxis=l.getValueAxis();

    Point p=null;

    if (domAxis.getPosition().getOrientation()==ORIENTATION.HORIZONTAL)
    {
      p=new Point(domAxis.logicalToScreen( domVal ), valAxis.logicalToScreen( valVal ));
    }
    else
    {
      p=new Point(valAxis.logicalToScreen( valVal ), domAxis.logicalToScreen( domVal ));
    }
    return p;
  }
