...
import org.kalypso.swtchart.configuration.parameters.impl.GregorianCalendarParser;
 ...
 * @author burtscher
 *
 * Layer Provider for precipitation data from O&M data;
 * it's looking for a feature named "niederschlagsmessung", then tries to
 * transform it into an IObservation, creates a NiederschlagLayer
 *  and uses the result components which go by the name "Datum" (domain data) and
 *  "Niederschlag" (value data) as data input for the layer; the NiederschlagLayer draws
 *  its data as bar chart
 *
 ...
 *  fixedPoint:     starting time (e.g. 2006-07-01T00:00:00Z) of any possible bar within the chart;
...
public class NiederschlagLayerProvider implements ILayerProvider
{
  ...
  public IChartLayer getLayer( URL context )
  {
      ...
      String url = ph.getParameterValue( "url", "" );
      String observationId=ph.getParameterValue( "observationId", "");
      try
      {
        ...
        String valueAxisId= ( (AxisType) m_lt.getAxes().getValueAxisRef().getRef()).getName();
          ...
          for( int i = 0; i < comps.length; i++ )
          {
            /*
             * TODO:
             * das sollte nicht hart  codiert sein - besser parameter verwenden
             */
            IComponent comp = comps[i];
            Logger.trace( "Component: " + comp.getName() );
            Logger.trace( "Component: " + comp.getDescription() );
            Logger.trace( "Component: " + comp.getValueTypeName() );
          ...
          GregorianCalendar fixedPoint=ph.getParsedParameterValue( "fixedPoint", "2006-10-10 00:00", new GregorianCalendarParser() );
          long barInterval = ph.getParsedParameterValue( "barInterval", (new Integer(24 * 60 * 60 * 1000)).toString(), new NumberParser() ).longValue();


          icl = new NiederschlagLayer( result, map.get( "domain" ), map.get( "value" ), domAxis, valAxis, fixedPoint, barInterval );
